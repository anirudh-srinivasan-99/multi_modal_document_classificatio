import pytest
import torch
from transformers import BatchEncoding

from src.model_builder.vision_classifier import VisionClassifier
from src.config.constants import ModelName

@pytest.fixture
def vision_fe_params() -> tuple[str, int, bool]:
    return (ModelName.EFFICIENT_NET_V2, 1024, False)


@pytest.fixture
def learning_rate() -> float:
    return 0.01


@pytest.fixture
def num_classes() -> int:
    return 10


@pytest.fixture
def vision_classifier(
    vision_fe_params: tuple[str, int, bool],
    learning_rate: float,
    num_classes: int
) -> VisionClassifier:
    vmn, vpd, vbt = vision_fe_params
    return VisionClassifier(
        vision_model_name=vmn,
        vision_projection_dimension=vpd,
        vision_backbone_trainable=vbt,
        num_classes=num_classes,
        learning_rate=learning_rate
    )


@pytest.fixture
def batch_size() -> int:
    return 8


@pytest.fixture
def batch_data(
    batch_size: int,
    num_classes: int,
    vision_classifier: VisionClassifier
) -> tuple[torch.Tensor, BatchEncoding, torch.Tensor]:
    h, w = vision_classifier.vision_fe.input_size
    max_seq_len = 512
    images = torch.randn(batch_size, 3, h, w)
    tokens = BatchEncoding(
        {
            'input_ids': torch.randint(0, 1000, (batch_size, max_seq_len)),
            'attention_mask': torch.ones(batch_size, max_seq_len) 
        }
    )
    labels = torch.randint(0, num_classes, (batch_size, ))
    return images, tokens, labels


def test_classifier_forward_pass(
    batch_data: tuple[torch.Tensor, BatchEncoding],
    batch_size: int,
    num_classes: int,
    vision_classifier: VisionClassifier
) -> None:
    images, tokens, _ = batch_data

    logits = vision_classifier(images, tokens)
    
    # Asserts if the shape is as expected.
    assert logits.shape == (batch_size, num_classes)

    # Asserts that the output vector is differentiable.
    assert logits.requires_grad is True


def test_training_step_logic(
    batch_data: tuple[torch.Tensor, BatchEncoding],
    vision_classifier: VisionClassifier
) -> None:
    loss = vision_classifier.training_step(batch_data, 0)

    # Asserts if the loss is of expected type, differentiable.
    #   and is not NaN.
    assert isinstance(loss, torch.Tensor)
    assert loss.requires_grad is True
    assert not torch.isnan(loss)
    
    # Run end of epoch to verify reset logic
    vision_classifier.on_train_epoch_end()

    # Verify metrics zeroed out
    for metric in vision_classifier.train_metrics.values():
        assert torch.all(metric.tp == 0)


def test_parameter_trainability(
    vision_fe_params: tuple[str, int, bool],
    vision_classifier: VisionClassifier
) -> None:
    _, _, vbt = vision_fe_params

    for params in vision_classifier.vision_fe.backbone.parameters():
        assert params.requires_grad is vbt
    
    for params in vision_classifier.vision_fe.projection_head.parameters():
        assert params.requires_grad is True
    
    for params in vision_classifier.classification_head.parameters():
        assert params.requires_grad is True

def test_validation_lifecycle(
    batch_size: int,
    batch_data: tuple[torch.Tensor, BatchEncoding, torch.Tensor],
    vision_classifier: VisionClassifier
) -> None:
    vision_classifier.validation_step(batch_data, 0)

    val_results = vision_classifier.val_metrics.compute()
    assert 0.0 <= val_results['val_MulticlassAccuracy'] <= 1.0
    
    vision_classifier.on_validation_epoch_end()

    # Verify metrics zeroed out
    for metric in vision_classifier.val_metrics.values():
        assert torch.all(metric.tp == 0)
    assert torch.all(vision_classifier.val_cm.confmat == 0)


def test_test_lifecycle(
    batch_size: int,
    batch_data: tuple[torch.Tensor, BatchEncoding, torch.Tensor],
    vision_classifier: VisionClassifier
) -> None:
    vision_classifier.test_step(batch_data, 0)

    test_results = vision_classifier.test_metrics.compute()
    assert 0.0 <= test_results['test_MulticlassAccuracy'] <= 1.0
    
    vision_classifier.on_test_epoch_end()

    for metric in vision_classifier.train_metrics.values():
        assert torch.all(metric.tp == 0)
    assert torch.all(vision_classifier.test_cm.confmat == 0)