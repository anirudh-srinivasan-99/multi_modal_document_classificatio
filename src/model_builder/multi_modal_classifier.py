...
class MultiModalClassifier(L.LightningModule):
    def __init__(
        self,
        vision_model_name: str, 
        vision_projection_dimension: int,
        vision_backbone_trainable: bool,
        ...
        """
        Initialize the MultiModalClassifier.

        :param vision_model_name: Name of the vision backbone (e.g., from timm).
        :type vision_model_name: str
        :param vision_projection_dimension: Output dimension of the vision feature extractor.
        :type vision_projection_dimension: int
        :param vision_backbone_trainable: Whether to update weights of the vision backbone.
        :type vision_backbone_trainable: bool
        ...
        self.vision_fe: VisionFeatureExtractor = VisionFeatureExtractor(
            backbone_model_name=vision_model_name,
            projection_dimension=vision_projection_dimension,
            backbone_trainable=vision_backbone_trainable
        )
        ...
        self.classification_head = ClassificationHead(
            input_dimensions=(vision_projection_dimension + language_projection_dimension),
    ...
    def forward(self, images: torch.Tensor, tokens: BatchEncoding) -> torch.Tensor:
        ...
        :rtype: torch.Tensor
        """
        vision_features = self.vision_fe(images)
        language_features = self.language_fe(tokens)

        fusion_features = torch.cat((vision_features, language_features), dim=1)

        return self.classification_head(fusion_features)
